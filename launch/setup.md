...
- for this project, the domain is `fozeus.me`
...
```text
https://web.archive.org/web/20160328162852/fozeus.me/
```
...
- Railway project: `fozeus.me`
- Railway service: `fozeus.me`
- Railway temporary domain: `https://fozeusme-production.up.railway.app`
- Custom domain: `fozeus.me`
...
```bash
railway init -n fozeus.me -w <workspace-id> --json
railway add --service fozeus.me --json
railway up --service fozeus.me --detach --verbose
railway domain --service fozeus.me --json
railway variable set SITE_URL=https://fozeus.me --service fozeus.me --environment production --skip-deploys
```

If `railway domain fozeus.me ...` returns `Unauthorized` even after `railway login`, use the Railway dashboard or Railway GraphQL API to create the custom domain. In this run, the CLI was intermittently authenticated for `status` but unauthorized for `domain`.
...
Railway gave these records for `fozeus.me`:
...
Zone > DNS > Edit
Zone > Zone > Read
Zone Resources: Include > Specific zone > fozeus.me
```
...
```bash
curl -s "https://cloudflare-dns.com/dns-query?name=fozeus.me&type=A" \
  -H "accept: application/dns-json"

curl -s "https://cloudflare-dns.com/dns-query?name=_railway-verify.fozeus.me&type=TXT" \
  -H "accept: application/dns-json"
```
